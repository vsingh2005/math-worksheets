...
\begin{tcolorbox}[colback=yellow!10,colframe=yellow!50!black,boxrule=0.8pt,arc=3mm]
\textbf{Quick Check: Which one looks proportional?}

Without calculating anything, make a prediction.
...
\end{tabular}
\end{enumerate}
Circle your prediction. Then we'll learn how to prove it.
\end{tcolorbox}
\vspace{4mm}
\begin{tcolorbox}[colback=red!5,colframe=red!60!black,arc=3mm]
\textbf{Common Mistake}

A relationship is \textbf{not} proportional just because both numbers increase.

The ratio between the quantities must stay the same every time.
\end{tcolorbox}
...
\begin{tikzpicture}
    ...
    \node at (1.1,1.5) {5 apples};

    % Right box
    \draw (3,0) rectangle (5.2,3);
    \node at (4.1,1.5) {10 apples};
\end{tikzpicture}
\end{center}
